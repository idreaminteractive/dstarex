...
defmodule Router do
  ...
  post "/test" do
    conn = DstarEx.SSE.new_sse(conn)
    {:ok, conn, signals} = DstarEx.SSE.read_signals(conn)

    Enum.reduce(signals["events"], conn, fn event, conn ->
      case event["type"] do
        "executeScript" ->
          DstarEx.SSE.execute_script(conn, event["script"],
        ...
        "patchElements" ->
          DstarEx.SSE.patch_elements(conn, event["elements"],
            event_id: event["eventId"],
            retry_duration: event["retryDuration"],
